...
export default function LedgerModal({ row, onClose }) {
  const entries = entriesForAccount(row.classificacao).sort((a, b) => String(a.data).localeCompare(String(b.data)));
        ...
        <div className="flex items-start justify-between gap-4 border-b border-line p-6">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[13px] text-ink-400">{row.classificacao} · {entries.length} lançamentos</p>
            </div>
          </div>
          <button
            type="button"
